    ...
    <div class="py-4 admin-page-info">
        ...
        <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center py-4">
            <div class="d-block mb-4 mb-md-0">
                ...
                <h2 class="h4"> <i class="fa-solid fa-handshake-simple text-primary"></i> Responsibilities List</h2>
                <p class="mb-0">You can manage this table  and do all opration system create , show, edit and delete</p>
            </div>
            <div class="btn-toolbar mb-2 mb-md-0"><a href="{{ route('admin.responsibility.create') }}"
                    class="btn btn-sm btn-primary d-inline-flex align-items-center"> <i class="fa-solid fa-plus"></i> &nbsp; New Responsibility</a>
            </div>
        </div>
        ...
        <div class="table-settings mb-4">
            <div class="row justify-content-between align-items-center">
                ...
                <div class="col-9 col-lg-8 d-md-flex">
                    <form action="{{ route('admin.responsibility.search') }}" method="POST" class="input-group me-2 me-lg-3 fmxw-400">
                        ...
                        placeholder="Search responsibilities by title"  value='{{ Request::input('search') }}' autocomplete="off" maxlength="55" required/>
                        @error('search')
                        ...
                        <div class="btn-group">
                            <div class="dropdown me-1">
                                ...
                                <div class="dynamic-pagination dropdown-menu dropdown-menu-end pb-0">
                                    ...
                                    <a class="dropdown-item {{ Request::is('*/perPage/10') ? 'active' : '' }} {{ Request::is('admin/responsibility') ? 'active' : '' }}"
                                        href="{{ route('admin.responsibility.perPage', 10) }}"> 10 </a>
                                    <a class="dropdown-item {{ Request::is('*/perPage/30') ? 'active' : '' }}"
                                        href="{{ route('admin.responsibility.perPage', 30) }}"> 30 </a>
                                    <a class="dropdown-item {{ Request::is('*/perPage/50') ? 'active' : '' }}"
                                        href="{{ route('admin.responsibility.perPage', 50) }}"> 50 </a>
                                    <a class="dropdown-item {{ Request::is('*/perPage/100') ? 'active' : '' }}"
                                        href="{{ route('admin.responsibility.perPage', 100) }}"> 100 </a>
        ...
        @else
            ...
            <div class="card card-body shadow border-0 table-wrapper table-responsive">
                ...
                <!----------- multi Action ------------->
                <form id="multi-action-form" action="{{ route("admin.responsibility.multiAction") }}" method="POST">
                    <div class="pb-3">
                        ...
                        <select id="select-action" class="form-select fmxw-200" name="action" aria-label="Message select example" style="display:inline">
                            ...
                            <option value="delete"> Delete Responsibility</option>
                        </select>
                    ...
                    <table class="table responsibility-table table-hover align-items-center index-table">
                        <thead>
                            <tr>
                        ...
                        <tbody>
                            @foreach ($responsibilities as $responsibility)
                                <tr>
                                    <td>
                                        <div class="form-check dashboard-check">
                                            <input name="id[]" value="{{ $responsibility->id }}" class="form-check-input checkbox-head check-item"  type="checkbox">
                                            <label class="form-check-label" for="userCheck55">  </label>
                                        </div>
                                    </td>
                                    <td><a href="{{ route('admin.responsibility.show', $responsibility->id) }}" class="d-flex align-items-center">
                                            <div class="d-block">
                                                <span class="fw-bold">
                                                    @if ( strlen($responsibility->title) > 30 )
                                                        {{ Str::ucfirst( substr( $responsibility->title , 0, 30 )) }}...
                                                    @else
                                                        {{  Str::ucfirst( $responsibility->title ) }}
                                                    @endif
                                                </span>
                                                <div class="small text-gray">{{ $responsibility->email }}</div>
                                            </div>
                                        </a></td>
                                    <td class="check-icons">
                                        @if ( $responsibility->visibility === "0" )
                                    ...
                                    <td><span class="fw-normal">{{ $responsibility->created_at }}</span></td>
                                    <td class="actions">
                                        <a href="{{ route('admin.responsibility.show', $responsibility->id) }}" class="text-tertiary"> <i
                                                class="fa-solid fa-eye fa-lg"></i> </a>
                                        <a href="{{ route('admin.responsibility.edit', $responsibility->id) }}" class="text-info"> <i
                                                class="fa-solid fa-pen-to-square fa-lg"></i> </a>
                                        <a href="{{ route('admin.responsibility.destroy', $responsibility->id) }}" class="text-info delete-record">
                    ...
                    <div
                        ...
                        <div class="d-flex justify-content-center">
                            {{ $responsibilities->links('pagination::bootstrap-4') }}
                        </div>
                        <div class="fw-normal small mt-4 mt-lg-0">
                            Showing <b>{{ $responsibilities->firstItem() }}</b> to <b>{{ $responsibilities->lastItem() }}</b>
                            of total <b>{{ $responsibilities->total() }}</b> entries
                        </div>
                    </div>
                </form>
            </div>
