...
@section('content')
    <div class="py-4 admin-page-info">
        ...
        <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center py-4">
            ...
            <div class="btn-toolbar mb-2 mb-md-0"><a href="{{ route('admin.offer.create') }}"
                    class="btn btn-sm btn-primary d-inline-flex align-items-center"> <i class="fa-solid fa-plus"></i> &nbsp; New Offer</a>
            </div>
        </div>
        ...
        <div class="table-settings mb-4">
            <div class="row justify-content-between align-items-center">
                ...
                <div class="col-9 col-lg-8 d-md-flex">
                    <form action="{{ route('admin.offer.search') }}" method="POST" class="input-group me-2 me-lg-3 fmxw-400">
                        ...
                        placeholder="Search offers by title"  value='{{ Request::input('search') }}' autocomplete="off" maxlength="55" required/>
                        @error('search')
                        ...
                        <div class="btn-group">
                            <div class="dropdown me-1">
                                ...
                                <div class="dynamic-pagination dropdown-menu dropdown-menu-end pb-0">
                                    ...
                                    <a class="dropdown-item {{ Request::is('*/perPage/10') ? 'active' : '' }} {{ Request::is('admin/offer') ? 'active' : '' }}"
                                        href="{{ route('admin.offer.perPage', 10) }}"> 10 </a>
                                    <a class="dropdown-item {{ Request::is('*/perPage/30') ? 'active' : '' }}"
                                        href="{{ route('admin.offer.perPage', 30) }}"> 30 </a>
                                    <a class="dropdown-item {{ Request::is('*/perPage/50') ? 'active' : '' }}"
                                        href="{{ route('admin.offer.perPage', 50) }}"> 50 </a>
                                    <a class="dropdown-item {{ Request::is('*/perPage/100') ? 'active' : '' }}"
                                        href="{{ route('admin.offer.perPage', 100) }}"> 100 </a>
        ...
        @else
            ...
            <div class="card card-body shadow border-0 table-wrapper table-responsive">
                ...
                <!----------- multi Action ------------->
                <form id="multi-action-form" action="{{ route("admin.offer.multiAction") }}" method="POST">
                    <div class="pb-3">
                    ...
                    <table class="table offer-table table-hover align-items-center index-table">
                        <thead>
                            <tr>
                                ...
                                <th class="border-bottom">Summary</th>
                        ...
                        <tbody>
                            @foreach ($offers as $offer)
                                <tr>
                                    <td>
                                        <div class="form-check dashboard-check">
                                            <input name="id[]" value="{{ $offer->id }}" class="form-check-input checkbox-head check-item"  type="checkbox">
                                            <label class="form-check-label" for="userCheck55">  </label>
                                        </div>
                                    </td>
                                    <td><a href="{{ route('admin.offer.show', $offer->id) }}" class="d-flex align-items-center">
                                            <div class="d-block">
                                                <span class="fw-bold">
                                                    @if ( strlen($offer->title) > 30 )
                                                        {{ Str::ucfirst( substr( $offer->title , 0, 30 )) }}...
                                                    @else
                                                        {{  Str::ucfirst( $offer->title ) }}
                                                    @endif
                                                </span>
                                            </div>
                                        </a></td>
                                    <td>
                                        <div class="d-block">
                                            <span class="fw-bold">
                                                @if ( strlen($offer->summary) > 30 )
                                                    {{ Str::ucfirst( substr( $offer->summary , 0, 30 )) }}...
                                                @else
                                                    {{  Str::ucfirst( $offer->summary ) }}
                                                @endif
                                            </span>
                                        </div>
                                    </td>
                                    <td><span class="fw-normal">{{ $offer->created_at }}</span></td>
                                    <td class="actions">
                                        <a href="{{ route('admin.offer.show', $offer->id) }}" class="text-tertiary"> <i
                                                class="fa-solid fa-eye fa-lg"></i> </a>
                                        <a href="{{ route('admin.offer.edit', $offer->id) }}" class="text-info"> <i
                                                class="fa-solid fa-pen-to-square fa-lg"></i> </a>
                                        <a href="{{ route('admin.offer.destroy', $offer->id) }}" class="text-info delete-record">
                    ...
                    <div
                        ...
                        <div class="fw-normal small mt-4 mt-lg-0">
                            Showing <b>{{ $offers->firstItem() }}</b> to <b>{{ $offers->lastItem() }}</b>
                            of total <b>{{ $offers->total() }}</b> entries
                        </div>
                    </div>
                </form>
            </div>
