    ...
    <div class="py-4 admin-page-info">
        ...
        <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center py-4">
            <div class="d-block mb-4 mb-md-0">
                ...
                <h2 class="h4"> <i class="fa-solid fa-file-lines text-primary"></i> Orders List</h2>
            ...
            @if (isset($offersCount) && $offersCount > 0)
                <div class="btn-toolbar mb-2 mb-md-0"><a href="{{ route('admin.order.create') }}"
                        class="btn btn-sm btn-primary d-inline-flex align-items-center"> <i class="fa-solid fa-plus"></i> &nbsp; New Order</a>
                </div>
            @endif
        ...
        <div class="table-settings mb-4">
            <div class="row justify-content-between align-items-center">
                ...
                <div class="col-9 col-lg-8 d-md-flex">
                    <form action="{{ route('admin.order.search') }}" method="POST" class="input-group me-2 me-lg-3 fmxw-400">
                        ...
                        placeholder="Search orders by email"  value='{{ Request::input('search') }}' autocomplete="off" maxlength="55" required/>
                        @error('search')
                        ...
                        <div class="btn-group">
                            <div class="dropdown me-1">
                                ...
                                <div class="dynamic-pagination dropdown-menu dropdown-menu-end pb-0">
                                    ...
                                    <a class="dropdown-item {{ Request::is('*/perPage/10') ? 'active' : '' }} {{ Request::is('admin/order') ? 'active' : '' }}"
                                        href="{{ route('admin.order.perPage', 10) }}"> 10 </a>
                                    <a class="dropdown-item {{ Request::is('*/perPage/30') ? 'active' : '' }}"
                                        href="{{ route('admin.order.perPage', 30) }}"> 30 </a>
                                    <a class="dropdown-item {{ Request::is('*/perPage/50') ? 'active' : '' }}"
                                        href="{{ route('admin.order.perPage', 50) }}"> 50 </a>
                                    <a class="dropdown-item {{ Request::is('*/perPage/100') ? 'active' : '' }}"
                                        href="{{ route('admin.order.perPage', 100) }}"> 100 </a>
        ...
        @else
            ...
            <div class="card card-body shadow border-0 table-wrapper table-responsive">
                ...
                <!----------- multi Action ------------->
                <form id="multi-action-form" action="{{ route("admin.order.multiAction") }}" method="POST">
                    <div class="pb-3">
                    ...
                    <table class="table order-table table-hover align-items-center index-table">
                        <thead>
                            <tr>
                                ...
                                <th class="border-bottom">Offer</th>
                        ...
                        <tbody>
                            @foreach ($orders as $order)
                                <tr>
                                    <td>
                                        <div class="form-check dashboard-check">
                                            <input name="id[]" value="{{ $order->id }}" class="form-check-input checkbox-head check-item"  type="checkbox">
                                            <label class="form-check-label" for="userCheck55">  </label>
                                        </div>
                                    </td>
                                    <td><a href="{{ route('admin.order.show', $order->id) }}" class="d-flex align-items-center">
                                            <div class="d-block">
                                                <span class="fw-bold">
                                                    @if ( strlen($order->email) > 30 )
                                                        {{ Str::ucfirst( substr( $order->email , 0, 30 )) }}...
                                                    @else
                                                        {{  Str::ucfirst( $order->email ) }}
                                                    @endif
                                                </span>
                                            </div>
                                        </a></td>
                                    <td><span class="fw-normal">
                                        <span class="fw-bold">
                                            @if ( strlen($order->offer->title) > 30 )
                                                {{ Str::ucfirst( substr( $order->offer->title , 0, 30 )) }}...
                                            @else
                                                {{  Str::ucfirst( $order->offer->title ) }}
                                            @endif
                                        </span>
                                    </span></td>
                                    <td><span class="fw-normal">{{ $order->created_at }}</span></td>
                                    <td class="actions">
                                        <a href="{{ route('admin.order.show', $order->id) }}" class="text-tertiary"> <i
                                                class="fa-solid fa-eye fa-lg"></i> </a>
                                        <a href="{{ route('admin.order.edit', $order->id) }}" class="text-info"> <i
                                                class="fa-solid fa-pen-to-square fa-lg"></i> </a>
                                        <a href="{{ route('admin.order.destroy', $order->id) }}" class="text-info delete-record">
