    ...
    <div class="py-4 admin-page-info">
        ...
        <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center py-4">
            <div class="d-block mb-4 mb-md-0">
                ...
                <h2 class="h4"> <i class="fa-solid fa-file-lines text-primary"></i> Appointments List</h2>
            ...
            @if (isset($membersCount) && $membersCount > 0)
                <div class="btn-toolbar mb-2 mb-md-0"><a href="{{ route('admin.appointment.create') }}"
                        class="btn btn-sm btn-primary d-inline-flex align-items-center"> <i class="fa-solid fa-plus"></i> &nbsp; New Appointment</a>
                </div>
            @endif
        ...
        <div class="table-settings mb-4">
            <div class="row justify-content-between align-items-center">
                ...
                <div class="col-9 col-lg-8 d-md-flex">
                    <form action="{{ route('admin.appointment.search') }}" method="POST" class="input-group me-2 me-lg-3 fmxw-400">
                        ...
                        placeholder="Search appointments by Username"  value='{{ Request::input('search') }}' autocomplete="off" maxlength="55" required/>
                        @error('search')
                        ...
                        <div class="btn-group">
                            <div class="dropdown me-1">
                                ...
                                <div class="dynamic-pagination dropdown-menu dropdown-menu-end pb-0">
                                    ...
                                    <a class="dropdown-item {{ Request::is('*/perPage/10') ? 'active' : '' }} {{ Request::is('admin/appointment') ? 'active' : '' }}"
                                        href="{{ route('admin.appointment.perPage', 10) }}"> 10 </a>
                                    <a class="dropdown-item {{ Request::is('*/perPage/30') ? 'active' : '' }}"
                                        href="{{ route('admin.appointment.perPage', 30) }}"> 30 </a>
                                    <a class="dropdown-item {{ Request::is('*/perPage/50') ? 'active' : '' }}"
                                        href="{{ route('admin.appointment.perPage', 50) }}"> 50 </a>
                                    <a class="dropdown-item {{ Request::is('*/perPage/100') ? 'active' : '' }}"
                                        href="{{ route('admin.appointment.perPage', 100) }}"> 100 </a>
        ...
        @else
            ...
            <div class="card card-body shadow border-0 table-wrapper table-responsive">
                ...
                <!----------- multi Action ------------->
                <form id="multi-action-form" action="{{ route("admin.appointment.multiAction") }}" method="POST">
                    <div class="pb-3">
                    ...
                    <table class="table appointment-table table-hover align-items-center index-table">
                        <thead>
                            <tr>
                                ...
                                <th class="border-bottom">Doctor</th>
                        ...
                        <tbody>
                            @foreach ($appointments as $appointment)
                                <tr>
                                    <td>
                                        <div class="form-check dashboard-check">
                                            <input name="id[]" value="{{ $appointment->id }}" class="form-check-input checkbox-head check-item"  type="checkbox">
                                            <label class="form-check-label" for="userCheck55">  </label>
                                        </div>
                                    </td>
                                    <td><a href="{{ route('admin.appointment.show', $appointment->id) }}" class="d-flex align-items-center">
                                            <div class="d-block">
                                                <span class="fw-bold">
                                                    @if ( strlen($appointment->username) > 30 )
                                                        {{ Str::ucfirst( substr( $appointment->username , 0, 30 )) }}...
                                                    @else
                                                        {{  Str::ucfirst( $appointment->username ) }}
                                                    @endif
                                                </span>
                                            </div>
                                        </a></td>
                                    <td><span class="fw-normal">{{ $appointment->member->name }}</span></td>
                                    <td><span class="fw-normal">{{ $appointment->date }}</span></td>
                                    <td class="actions">
                                        <a href="{{ route('admin.appointment.show', $appointment->id) }}" class="text-tertiary"> <i
                                                class="fa-solid fa-eye fa-lg"></i> </a>
                                        <a href="{{ route('admin.appointment.edit', $appointment->id) }}" class="text-info"> <i
                                                class="fa-solid fa-pen-to-square fa-lg"></i> </a>
                                        <a href="{{ route('admin.appointment.destroy', $appointment->id) }}" class="text-info delete-record">
                    ...
                    <div
                        ...
                        <div class="fw-normal small mt-4 mt-lg-0">
                            Showing <b>{{ $appointments->firstItem() }}</b> to <b>{{ $appointments->lastItem() }}</b>
                            of total <b>{{ $appointments->total() }}</b> entries
                        </div>
                    </div>
                </form>
            </div>
