    ...
    <div class="py-4 admin-page-info">
        ...
        <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center py-4">
            <div class="d-block mb-4 mb-md-0">
                ...
                <h2 class="h4"> <i class="fa-solid fa-envelope text-primary"></i> Mails List</h2>
                <p class="mb-0">You can manage this table  and do all opration system create , show, edit and delete</p>
            </div>
            <div class="btn-toolbar mb-2 mb-md-0"><a href="{{ route('admin.newsletter.create') }}"
                    class="btn btn-sm btn-primary d-inline-flex align-items-center"> <i class="fa-solid fa-paper-plane"></i> &nbsp; Send New Mail </a>
            </div>
        </div>
        ...
        <div class="table-settings mb-4">
            <div class="row justify-content-between align-items-center">
                ...
                <div class="col-9 col-lg-8 d-md-flex">
                    <form action="{{ route('admin.newsletter.search') }}" method="POST" class="input-group me-2 me-lg-3 fmxw-400">
                        ...
                        placeholder="Search mails by subject"  value='{{ Request::input('search') }}' autocomplete="off" maxlength="55" required/>
                        @error('search')
                        ...
                        <div class="btn-group">
                            <div class="dropdown me-1">
                                ...
                                <div class="dynamic-pagination dropdown-menu dropdown-menu-end pb-0">
                                    ...
                                    <a class="dropdown-item {{ Request::is('*/perPage/10') ? 'active' : '' }} {{ Request::is('admin/newsletter') ? 'active' : '' }}"
                                        href="{{ route('admin.newsletter.perPage', 10) }}"> 10 </a>
                                    <a class="dropdown-item {{ Request::is('*/perPage/30') ? 'active' : '' }}"
                                        href="{{ route('admin.newsletter.perPage', 30) }}"> 30 </a>
                                    <a class="dropdown-item {{ Request::is('*/perPage/50') ? 'active' : '' }}"
                                        href="{{ route('admin.newsletter.perPage', 50) }}"> 50 </a>
                                    <a class="dropdown-item {{ Request::is('*/perPage/100') ? 'active' : '' }}"
                                        href="{{ route('admin.newsletter.perPage', 100) }}"> 100 </a>
        ...
        @else
            ...
            <div class="card card-body shadow border-0 table-wrapper table-responsive">
                ...
                <!----------- multi Action ------------->
                <form id="multi-action-form" action="{{ route("admin.newsletter.multiAction") }}" method="POST">
                    <div class="pb-3">
                    ...
                    <table class="table newsletter-table table-hover align-items-center index-table">
                        <thead>
                            <tr>
                        ...
                        <tbody>
                            @foreach ($newsletters as $newsletter)
                                <tr>
                                    <td>
                                        <div class="form-check dashboard-check">
                                            <input name="id[]" value="{{ $newsletter->id }}" class="form-check-input checkbox-head check-item"  type="checkbox">
                                            <label class="form-check-label" for="userCheck55">  </label>
                                        </div>
                                    </td>
                                            ...
                                            <div class="d-block">
                                                <span class="fw-bold">
                                                    @if ( strlen($newsletter->subject) > 30 )
                                                        {{ Str::ucfirst( substr( $newsletter->subject , 0, 30 )) }}...
                                                    @else
                                                        {{  Str::ucfirst( $newsletter->subject ) }}
                                                    @endif
                                                </span>
                                                <div class="small text-gray">{{ $newsletter->email }}</div>
                                            </div>
                                        </a></td>
                                    <td><span class="fw-normal">{{ $newsletter->created_at }}</span></td>
                                    <td class="actions">
                                        <a href="{{ route('admin.newsletter.show', $newsletter->id) }}" class="text-tertiary"> <i
                                                class="fa-solid fa-eye fa-lg"></i> </a>
                                        <a href="{{ route('admin.newsletter.destroy', $newsletter->id) }}" class="text-info delete-record">
                    ...
                    <div
                        ...
                        <div class="fw-normal small mt-4 mt-lg-0">
                            Showing <b>{{ $newsletters->firstItem() }}</b> to <b>{{ $newsletters->lastItem() }}</b>
                            of total <b>{{ $newsletters->total() }}</b> entries
                        </div>
                    </div>
                </form>
            </div>
